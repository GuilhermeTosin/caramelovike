-- Keep radius pagination on one stable result set.
-- Online businesses are included only when they have a reliable location
-- inside the requested radius, matching the frontend search rule.
-- Include the primary activity and its custom label in radius searches.
-- The activity id is intentionally searched as well as the custom text so
-- values such as "churrascaria" work for every existing and future record.
...
as $$
  ...
  filtered as (
    ...
    where
      ...
      and (
        coalesce(b.attendance_type, 'presencial') = 'online'
        or (
          nullif(trim(coalesce(b.street, '')), '') is not null
